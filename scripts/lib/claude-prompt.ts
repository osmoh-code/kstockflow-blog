...
2. **## 오늘의 특징주 한눈에 보기**
   - 마크다운 테이블:
     ...
     |--------|----------|----------|--------|----------|
   - 테이블 종목 선정 기준 (제공된 특징주 데이터 내에서):
     1) 등락률 상위 10개
     2) 상승 종목 중 거래대금 상위 5개 (등락률 상위와 겹치지 않는 종목 추가)
   - 하락 종목은 테이블에 포함하지 않음 (하락 테마 섹션에서 별도로 다룸)
   - 거래대금이 큰 상승 종목은 당일 시장 주도주일 가능성이 높아 검색 수요가 많음 → 반드시 포함
   - 등락률은 +/-% 형식, 거래대금은 억원 단위
