...
- DART 전자공시시스템의 투자설명서/증권신고서 전문 데이터를 기반으로 정확한 정보를 전달합니다.

## ⛔ 최우선 원칙: DART 공식 데이터 기반 작성 (절대 위반 금지)

### 데이터 소스 우선순위
1. DART 투자설명서/증권신고서 전문 (제공된 [섹션: xxx] 데이터) — 최우선
2. DART 기업개황 API 데이터 — 보조
3. 최신 뉴스 — 배경/동향 참고용

### 데이터 정확성 규칙
- 제공된 DART 데이터의 수치를 그대로 사용하세요. 임의로 변경·반올림·추정하지 마세요.
- 제공된 데이터에 없는 수치(공모가, 주식수, 경쟁률, 재무수치, 주주명, 지분율 등)는 절대 지어내지 마세요.
- 확인할 수 없는 정보는 "확인 필요"로 짧게 표시하세요.
- 종목코드, 주간사, 일정, 고유명사는 제공된 데이터와 정확히 일치해야 합니다.
- 이 원칙을 위반하면 투자자에게 실질적 피해를 줄 수 있습니다. 정확하지 않은 것보다 빈칸이 낫습니다.

### 섹션별 데이터 추출 필수 항목
제공된 데이터에서 아래 항목을 반드시 찾아서 본문에 포함하세요:

| 섹션 라벨 | 추출할 데이터 |
|-----------|--------------|
| [섹션: 공모개요] | 공모가(확정가/희망밴드), 공모주식수, 공모금액, 주간사, 상장시장, 청약일정, 상장예정일, 수요예측 경쟁률, 의무보유확약 비율 |
| [섹션: 재무제표] | 최근 3개년 매출액, 영업이익(손실), 당기순이익(손실), 영업이익률, 자산총계, 부채총계, 부채비율 |
| [섹션: 주주현황] | 최대주주명, 최대주주 지분율, 특수관계인 지분, 주요 VC/PE 투자자명과 지분율 |
| [섹션: 보호예수] | 보호예수 기간별(6개월/1년/즉시유통) 주식수와 비율, 총 유통제한 물량, 상장직후 유통가능 비율 |
| [섹션: 자금사용계획] | 운영자금/시설자금/채무상환 등 용도별 금액과 비율 |
| [섹션: 수요예측] | 기관투자자 수요예측 경쟁률, 확약비율, 가격대별 수요 분포 |
| [섹션: 사업내용] | 주요 사업 영역, 핵심 제품/서비스, 매출 구성 비율, 시장 점유율, 경쟁사 |
| [섹션: 위험요소] | 주요 투자 위험 요인 3~5개 |
...
## 콘텐츠 품질 기준
- 본문 최소 5,000자(한글 기준) 이상 — DART 전문 데이터가 제공되므로 충분히 길게 작성 가능
- 독창적 분석: 단순 사실 나열이 아닌, 데이터 간 관계를 파악한 심층 분석
  예: 부채비율 + 자금사용계획 → 재무건전성 평가
  예: 보호예수 해제 일정 + 유통물량 → 수급 리스크 분석
  예: 매출성장률 + PER 비교 → 밸류에이션 적정성 평가
- 투자 면책 고지 포함
...
- "투자설명서를 확인하세요", "공시를 참조하세요" 등 독자에게 떠넘기는 표현 — 당신이 이미 전문을 갖고 있으므로 직접 분석하세요
`;
...
`;
  ...
  if (ipoData) {
    prompt += `
## ⚠️ 핵심 데이터: DART 투자설명서/증권신고서 + 기업개황
아래 데이터는 DART 전자공시시스템에서 직접 수집한 공식 데이터입니다.
[섹션: xxx] 라벨이 붙은 데이터를 꼼꼼히 읽고, 각 섹션에서 필요한 수치를 정확히 추출하여 본문에 사용하세요.
...
⚠️ 중요: 각 섹션에서 어떤 DART 데이터를 사용해야 하는지 아래에 명시했습니다. 반드시 해당 데이터를 찾아서 포함하세요.

1. **## ${keyword} 공모 현황**
   📌 데이터 소스: [섹션: 공모개요], [섹션: 수요예측]
   - 마크다운 테이블로 공모 정보 정리:
     | 항목 | 내용 |
     |------|------|
     | 종목명 | |
...
3. **## ${keyword} 주요 재무 분석**
   📌 데이터 소스: [섹션: 재무제표]
   - 마크다운 테이블로 최근 3개년 재무 정리 (투자설명서에서 추출한 정확한 수치):
     | 구분 | 2022 | 2023 | 2024 |
     |------|------|------|------|
     | 매출액 | | | |
     | 영업이익(손실) | | | |
...
6. **## ${keyword} 유통주식 및 보호예수 물량 분석**
   ...
   - ⚠️ "투자설명서를 확인하라" 식의 떠넘기기 문구 절대 금지. 제공된 데이터에서 직접 추출하여 분석하세요.

7. **## ${keyword} 투자 포인트**
   - 3-5개의 핵심 투자 매력 포인트
   - 각 포인트별 근거와 구체적 수치 포함 (DART 데이터에서 추출)
   - ✔ 체크리스트 형식 활용

8. **## ${keyword} 리스크 포인트**
   📌 데이터 소스: [섹션: 위험요소]
   - 3-5개의 주요 리스크 요인 (투자설명서의 위험요소 섹션에서 추출)
   - 각 리스크의 영향도와 발생 가능성
   - ⚠️ 경고 형식 활용
...
## 추가 지침
- 전체 본문 최소 5,000자 이상 (DART 전문 데이터가 있으므로 충분히 상세하게)
- 볼드 절대 금지, mark 태그만 사용
- tags에 롱테일 키워드 포함
- 내부 링크 유도: "다른 신규 상장주 분석도 함께 참고하시기 바랍니다." 등
- ⚠️ 절대 금지 문구: "투자설명서를 확인하시기 바랍니다", "공시 자료를 참조", "증권신고서 본문을 통해 확인" 등 독자에게 떠넘기는 표현 금지
- 제공된 DART 데이터에서 직접 수치를 추출하여 분석하세요. 데이터가 부족한 항목만 "확인 필요"로 표시
- 취소선(~~텍스트~~) 절대 사용 금지
- 공모자금 사용계획 섹션을 새로 추가했습니다 — 투자설명서에 반드시 있는 핵심 정보이므로 빠짐없이 포함`;
