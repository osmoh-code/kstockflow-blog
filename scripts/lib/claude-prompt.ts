...
## GEO 최적화 규칙 (AI 검색엔진 인용 대응)
- 각 H2 섹션의 첫 문단(40~60단어)은 독립적으로 인용 가능한 자급자족형 요약문으로 작성
  → AI가 이 문단만 발췌해도 맥락이 완전히 전달되어야 함
  → "위에서 언급했듯이", "앞서 살펴본 것처럼" 같은 참조형 표현 사용 금지
- 150~200단어마다 구체적 통계/수치를 1개 이상 포함 (출처가 있는 데이터 우선)
  → "많은 기업" 대신 "전체 기업의 67%", "급등" 대신 "+12.5%" 등 구체적 수치 사용

## 국가·고유명사 표기 규칙 (매우 중요 — 약칭 금지)
- **국가 약칭은 반드시 가운뎃점(·)으로 구분**. 붙여 쓰지 마세요.
  - ✅ "미·이란", "미·중", "한·미", "미·이스라엘", "일·중"
  - ❌ "미이란", "미중", "한미", "미이스라엘", "일중" (붙여쓰기 절대 금지)
- 사용자가 키워드로 "미이란" 같은 붙여쓴 표기를 주어도 **본문에서는 반드시 "미·이란"으로 교정해서 서술**
- 기업명/고유명사는 사용자가 지정한 relatedStocks 배열의 철자와 **100% 동일하게** 사용.
  절대 오타 내지 마세요 (예: "금호건설"을 "금고건설"로 잘못 쓰는 일 없게).
...
## GEO 최적화 (AI 검색엔진 인용 대응)
- 글 첫 문단(40~60단어)에 오늘 시장의 핵심 결론을 독립적으로 인용 가능하게 작성
  예: "4월 16일 한국 증시에서 양자컴퓨팅 관련주가 전면 강세를 보였다. 드림시큐리티가 +14.2%, 케이씨에스가 +11.8% 급등하며 섹터를 주도했다."
- 각 섹터 분석도 첫 문장에 핵심 팩트(종목명+등락률) 배치 → AI가 발췌 인용 가능
...
## GEO 최적화 (AI 검색엔진 인용 대응)
- 서두 첫 문단(40~60단어)에 종목의 핵심 투자포인트를 독립적으로 인용 가능하게 요약
- 각 H2 섹션 첫 문단도 해당 섹션의 결론을 먼저 제시 (자급자족형 — "위에서 언급했듯이" 금지)
- 150~200단어마다 구체적 수치 포함 (공모가, 경쟁률, 매출액, 성장률 등)
...
2. **## 오늘의 특징주 한눈에 보기**
   - 마크다운 테이블:
     ...
     |--------|----------|----------|--------|----------|
   - **종목 선정**: 등락률 상위 20개 (상승 종목만, 하락 종목 제외). 그 외 종목 절대 추가 금지.
   - **정렬 순서: 반드시 등락률 내림차순** (가장 많이 오른 종목이 맨 위).
   - 등락률은 +/-% 형식, 거래대금은 억원 단위
   - **등락률과 거래대금은 반드시 [상승 종목 시세 데이터]에서 제공된 값을 그대로 사용. 절대 추정/지어내지 않음**
...
1. **## ${keyword} 관련주 핵심 요약**
   - 서론 없이 바로 본론으로 시작: 현재 벌어지고 있는 구체적 사건/이슈부터 서술 (누가, 언제, 무엇을, 왜)
   - ❌ "~어려우신가요?", "~고민이신가요?", "더 큰 문제는~" 같은 AI식 도입부 절대 금지
   - ❌ 문제-심화-해결(PAS) 패턴 사용 금지
   - 첫 문단부터 팩트 중심: 오늘 무슨 일이 일어났고, 왜 관련주가 움직이는지 설명
   - 왜 지금 이 주제가 중요한지 배경 설명 (2-3문단)
...
2. **## ${keyword} 시장 상세 분석**
   ...
   - **주제 특화 H3 소섹션 1~2개를 자율적으로 추가하세요** (중복 방지 핵심)
     → 주제의 고유 각도를 ### H3로 분리해서 깊이 분석
     예시 (이벤트성 주제): "### IPO 예상 일정과 국내 증시 연결 고리", "### 과거 유사 상장 사례 비교"
     예시 (정치/전쟁): "### 과거 중동 휴전 이후 재건주 6개월 성과", "### 현재 국면의 차별점"
     예시 (기술/산업): "### 기술 진화 단계와 현재 위치", "### 글로벌 vs 국내 밸류체인 위치"
     예시 (정책/규제): "### 법안 주요 조항과 수혜 구간", "### 과거 유사 정책의 주가 반응"
     ⚠️ 이 H3는 주제마다 달라야 합니다. 모든 글에 같은 제목 쓰면 안 됩니다.
...
4. **## ${keyword} 투자 시 체크포인트**
   - ⚠️ **반드시 이 주제에서만 유의미한 체크포인트 4~5개**를 작성 (모든 글이 같은 체크리스트 갖지 않도록)
   - ❌ **절대 금지 일반형 체크포인트 (boilerplate 감지 원인)**:
     · "단기 테마인지, 실적 개선 구간인지 구분"
     · "관련 지표 동반 확인"
     · "리스크 해소 시 급락 가능성 대비"
     · "PER, PBR 등 밸류에이션 지표 확인"
     · "거래량과 기관 매매 동향 확인"
     · "기술 경쟁력과 시장 점유율 검증"
     → 위 표현 그대로 쓰면 안 됩니다. 이들은 모든 주식 글에 해당돼서 Google이 boilerplate로 감지함.
   - ✅ **반드시 주제 고유 변수로 작성** — 이 주제에만 적용되는 이벤트·숫자·임계값·일정 포함
     **(이벤트성 주제 예 — IPO/상장/휴전/선거 등)**
       ✔ "SEC 예비심사 승인 vs 철회 시 영향 분기점 파악" (사건 기반)
       ✔ "락업 기간 6개월 해제 시점(2026-Q4)에 기존 투자자 물량 출회 위험"
       ✔ "상장 직후 첫 분기 실적에서 AI 위성 매출 비중 30% 이상 확인"
     **(정책/법안 주제 예)**
       ✔ "법안 본회의 통과 예상일(N월 N일) 전후 주가 반응"
       ✔ "시행령 공포 후 6개월 유예기간 → 실제 수혜 반영 시점"
       ✔ "예산 배정 규모 (N조원) 중 경쟁입찰 참여 가능 기업 비중"
     **(기술/산업 주제 예)**
       ✔ "차세대 표준(예: 112Gbps PAM4) 채택 시점에 따른 수혜 순서"
       ✔ "엔비디아 Rubin GPU 출시 시점(2026년 N월)과 맞물린 수주 공시"
       ✔ "경쟁 소재 대체 위험 (예: 실리콘 포토닉스 vs 기존 광모듈)"
     **(전쟁/분쟁 주제 예)**
       ✔ "분쟁 재개 시 물동량 감소가 해운 운임에 미치는 탄성"
       ✔ "재건 프로젝트 발주 국가별 한국 기업 참여 이력 (이라크 2003년 사례)"
       ✔ "원자재(철강/유가) 가격 변동 → 수주 마진 영향 민감도"
   - 각 항목은 **구체 숫자/시점/임계값** 포함, 1-2문장 부연 설명
   - 일반형 체크포인트는 1개까지만 허용 (나머지는 모두 주제 특화)
   - ⚠️ **mark 태그 일관성 필수 — 모든 체크포인트 항목에 동일 규칙 적용**:
     · 각 체크포인트 항목 **첫 문장의 핵심 키워드구**를 반드시 \`<mark>...</mark>\`로 감쌀 것
     · 한 체크포인트에만 mark 쓰고 다른 항목에 안 쓰면 안 됨 (기존 불일치 사고 원인)
     · ❌ 금지 — "첫째,", "둘째,", "1.", "2." 같은 서수/번호 사용 금지 (문단 구분만)
     · 각 항목은 빈 줄로 분리 (서수 없이)
     · 좋은 예: \`<mark>SEC 예비심사 진행 상황과 승인 vs 연기 시나리오</mark>별 영향을 분석해야 합니다. {부연설명}\`
     · 나쁜 예: \`첫째, 휴전 협정 체결 시점과 시차를 고려해야 합니다.\` (서수 사용 + mark 없음)

5. **## ${keyword} 관련주 투자 결론**
   - ⚠️ **아래 3가지 구성 중 하나를 주제에 맞게 골라서 자율 작성** (모든 글이 같은 결론 구조를 갖지 않도록)
     **(A) 시나리오 분기형** — 낙관/비관/중립 3개 시나리오별 주가 영향 시점·조건 명시
        예: "IPO가 6월 예정대로 진행되면…", "지연되면…", "중단되면…"
     **(B) 타임라인형** — 단기(1개월)/중기(3~6개월)/장기(1년+) 관전 포인트
        예: "1개월 내: 거래대금 5천억 유지 여부가 관건", "3개월: 실제 수주 공시…"
     **(C) 트레이딩 각도형** — 관련주 간 우선순위 + 진입/이탈 조건
        예: "실적 베이스는 A, 모멘텀 베이스는 B, 펀더멘탈 취약한 C는 단기만"
   - ❌ **절대 금지 상투구 (모든 글에 반복되어 boilerplate 감지됨)**:
     · "향후 주시할 포인트는~"
     · "~에 주목해야 할 시점입니다"
     · "투자자들의 관심이 집중되고 있습니다"
     · "중장기적인 기회"
     · "실제 ~와 연결될 수 있는 기업들을 선별해서 투자"
     · "변동성이 클 수 있지만"
     · "진정한 수혜주가 될 것입니다"
   - ✅ **반드시 포함**: 구체적 시점(월/분기), 구체적 숫자(수주 규모/실적 목표/밸류에이션), 관찰 가능한 트리거(실적 발표·법안 통과·수주 공시 등)
   - "함께 보면 좋은 분석 글" 섹션은 MDX에 작성하지 마세요 (RecommendedPosts 컴포넌트가 자동 렌더링)
   - 투자 면책 고지 (blockquote 형식): "> ※ 본 글은 정보 제공을 목적으로 하며, 투자의 책임은 투자자 본인에게 있습니다."

6. **## 자주 묻는 질문** (FAQ — 구글 리치 스니펫 노출용, 매우 중요!)
   - 반드시 **3~5개**의 질문과 답변을 포함하세요.
   - 형식: \`### Q. 질문내용\` 다음 줄에 답변 2-3문장
   - ⚠️ **질문은 주제마다 전부 다르게 작성하세요** (중복 색인 회피 핵심)
     · ❌ 금지: "관련주는 어떤 종목이 있나요?", "대장주는 무엇인가요?", "주가 전망은 어떤가요?"
        → 이 3개는 모든 글에 있어서 Google이 boilerplate로 감지합니다.
     · ✅ 권장: 이 주제에서만 물어볼 법한 **구체적·고유 질문**으로 작성
       예(스페이스X IPO): "머스크는 스페이스X 지분을 얼마나 보유하고 있나요?", "한국 개인투자자도 스페이스X 주식을 직접 살 수 있나요?", "상장 후 기존 스타링크 서비스에 변화가 있나요?"
       예(중동 휴전 건설): "이라크 재건 사업 규모는 얼마나 되나요?", "과거 휴전 직후 6개월간 건설주 평균 수익률은?", "수주 계약이 실제 매출로 잡히는 시점은 언제인가요?"
       예(AI 광통신): "실리콘 포토닉스와 기존 광통신의 차이는 무엇인가요?", "엔비디아 Rubin 세대에서 광통신 채택 비중이 얼마나 늘어나나요?"
     · 필요하면 일반형 질문 1개 정도는 OK, 나머지는 반드시 주제 특화
   - 답변은 간결하면서도 핵심 정보와 구체 수치 포함 (사실 기반)
