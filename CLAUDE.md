...
### 🎯 SERP CTR 최적화 (frontmatter title/description — 2026-05-06 재강화)

GSC Search Analytics 분석 결과: 노출 대비 클릭이 1/10 수준이면 색인이 아니라 **메타 CTR 문제**.

**hot-issues title 형식 (필수, 45~58자):**
```
{핵심키워드} {long-tail수식어} 관련주 한국 TOP {N} 2026 | {대장주1} +XX%·{대장주2} {모멘텀어}
```

**필수 요소 6개 (모두 포함):**
1. **검색 변형 long-tail 1개**: 기본 키워드 뒤에 "데이터센터", "수주", "수혜" 등 검색 변형 추가
2. **"한국" 1회**: GSC query에서 가장 빈번한 long-tail. 누락 시 `parseResponse`가 자동 삽입
3. **TOP {N} 1회만**: 두 번이면 빌더가 자동 차단
4. **연도(2026) 1회만**: 두 번이면 차단
5. **suffix에 대표 종목 1~2개 + 등락률 % 1개**: SERP CTR 가장 강함
6. **45~58자**: SERP 잘림 방지

**좋은 예 (목표 형식):**
- "AI 데이터센터 전력 관련주 한국 TOP 8 2026 | 대원전선 +26%·LS ELECTRIC 수혜주" (50자)
- "중동 재건 철강주 한국 TOP 7 2026 | 부국철강 +10%·현대제철 수주 모멘텀" (45자)
- "스테이블코인 결제 관련주 한국 TOP 6 2026 | 다날 +15%·미투온 결제 플랫폼 수혜" (48자)

**나쁜 예 (CTR 0~0.2% 사례):**
- "스페이스X 관련주 TOP 5 2026" (20자) — "한국"·long-tail·등락률 모두 누락
- "AI 전력 관련주 TOP 8 2026 | 대원전선·LS ELECTRIC·효성중공업 데이터센터 수혜주" (55자) — 노출 2,464 / 클릭 4 (CTR 0.16%, "한국" 누락 + 등락률 누락)
- "| 대장주·수혜주·테마주 총정리" 17자 boilerplate — 핵심 정보를 잘라먹음, 의미 없음

**parseResponse 자동 보정 (커밋 2026-05-06):**
- "한국" 키워드 누락 시 "관련주" 앞에 자동 삽입
- TOP N / 연도 2회 이상이면 자동 fallback 재조립
- 60자 초과 시 빌드 경고 (`validate-seo.ts`)
